...
interpreter = None
from power_monitor_thread import CPU_Power_Monitor_Thread
divider = '------------------------------------'
...
def inference(image_path,batch_size, model_path, num_of_runs):
    ...
    t1 = CPU_Power_Monitor_Thread(sleep_time = 0.1)
    start =  time.time()
    t1.start()
...
def main():
    ...
    logging.basicConfig(filename='logs/sc3_cpu_power_metrics_{}.log'.format(args.num_of_runs), level=logging.INFO)
