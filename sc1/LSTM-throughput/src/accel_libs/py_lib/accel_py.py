...
def parameters():
    ...
    oFRAQ_CONV = 4096
    MAX_BATCH = 256
    return [MTIMESTEPS,FEATURES,IUNITS,OUNITS,iFRAQ,oFRAQ,iFRAQ_CONV,oFRAQ_CONV,MAX_BATCH]
...
def init_buffers(batch, timesteps):
    PARAM=parameters()
    accel.lstm_io(int(timesteps), PARAM[0], PARAM[1], PARAM[2], PARAM[3], int(batch))
...
def fl2short_input(data, max_batch, max_steps, batch, timesteps, features, fraq):
    accel.fixp.restype = ctypes.c_short
    buf_data = numpy.empty((batch*max_steps*features),ctypes.c_short)
    for i in range(int(batch)):	
        for j in range(max_steps*features):	
            if (j < timesteps*features): 
                buf_data[i*(max_steps*features)+j] = accel.fixp(ctypes.c_float(data[i*(timesteps*features)+j]),fraq)
            else:  
                buf_data[i*(max_steps*features)+j] = 0
    return buf_data

def out_ptr(batch):
    PARAM = parameters()
    shp = ctypes.POINTER(ctypes.c_short)
    buf_res = numpy.zeros((PARAM[0]*PARAM[1]*int(batch)),ctypes.c_short)
    res_ptr = buf_res.ctypes.data_as(shp)
    return res_ptr

def in_ptr(data_in, batch, timesteps):
    PARAM = parameters()
    shp = ctypes.POINTER(ctypes.c_short)
    buf_data = fl2short_input(data_in, PARAM[8], PARAM[0], int(batch), int(timesteps), PARAM[1], PARAM[4])
    data_ptr = buf_data.ctypes.data_as(shp)
    return data_ptr

def lstm(data_ptr, res_ptr, wb_ptrs, batch, timesteps, flag):
    PARAM = parameters()
    accel.lstm_accel(data_ptr,res_ptr,wb_ptrs[0],wb_ptrs[1],wb_ptrs[2],wb_ptrs[3],wb_ptrs[4],wb_ptrs[5],wb_ptrs[6],wb_ptrs[7],wb_ptrs[8],wb_ptrs[9],int(timesteps), PARAM[0],flag,PARAM[1],PARAM[2],PARAM[3],int(batch))

def results(res_ptr, batch, timesteps):
    PARAM = parameters()
    R = numpy.empty(PARAM[0]*PARAM[1]*int(batch), float)
    for i in range(PARAM[0]*PARAM[1]*int(batch)):
            R[i] = (res_ptr[i]/PARAM[7])
    return R

def print_res(res_ptr, res, batch, timesteps):
    PARAM = parameters()
    #res = results(res_ptr, batch, timesteps)
    for i in range(int(batch)):
        if(res[i*PARAM[0]*PARAM[1]+(int(timesteps)-1)]>0.5):
            print("Attack detected : ",res[i*PARAM[0]*PARAM[1]+(int(timesteps)-1)])
        else:
            print("Normal process  : ",res[i*PARAM[0]*PARAM[1]+(int(timesteps)-1)])
    #return res

def print_res2(res_ptr, res, batch, timesteps,ret_count=3):
    PARAM = parameters()
    #res = results(res_ptr, batch, timesteps)
    ret=[]
    for i in range(int(batch)):
        ...
        if i < ret_count:
            print(curr)
            ret.append(curr)
    ret.append("................"+str(int(batch)-ret_count) + " more results omitted")
    return ret
