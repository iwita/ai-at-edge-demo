...
dtype row_vector_mul(const dtype w[MAX_MCOL], dtype h_x[MAX_MCOL], dtype b)
{
	...
	LOOP_MATRIX:for(int j=0; j<MAX_MCOL; j++) {
		#pragma HLS LOOP_TRIPCOUNT min = min_col max = max_col
...
void gate(dtype W[MAX_UNITS][MAX_MCOL], dtype b[MAX_UNITS], dtype h_x[MAX_MCOL], dtype out[MAX_UNITS], int type)
{
#pragma HLS ARRAY_PARTITION variable=W dim=2 complete
#pragma HLS ARRAY_PARTITION variable=b complete
#pragma HLS ARRAY_PARTITION variable=h_x complete
	dtype2 act_fun[MAX_UNITS];
	LOOP_ROW:for(int i=0; i<MAX_UNITS; ++i) {
		#pragma HLS LOOP_TRIPCOUNT min = min_units max = max_units
		#pragma HLS PIPELINE II = 1
		act_fun[i] = row_vector_mul(W[i], h_x, b[i]);
	}
        if(type==0)
	    array_sigmoid(act_fun, out);
        else
	    array_tanh(act_fun, out);
}

void forget_gate(dtype Wf[MAX_UNITS][MAX_MCOL], dtype bf[MAX_UNITS], dtype h_xf[MAX_MCOL], dtype out_f[MAX_UNITS])
{
...
void newcell_gate(dtype Wc[MAX_UNITS][MAX_MCOL], dtype bc[MAX_UNITS], dtype h_xc[MAX_MCOL], dtype out_c[MAX_UNITS])
{
...
void output_gate(dtype Wo[MAX_UNITS][MAX_MCOL], dtype bo[MAX_UNITS], dtype h_xo[MAX_MCOL], dtype out_o[MAX_UNITS])
{
...
void krnl_lstm(ap_int<512> *data, ap_int<512> *iWi, ap_int<512> *ibi, ap_int<512> *iWf, ap_int<512> *ibf, ap_int<512> *iWc, ap_int<512> *ibc, ap_int<512> *iWo, ap_int<512> *ibo, ap_int<512> *iW, ap_int<16> *ib, short int *res, int active_timesteps)
{
	...
	dtype Wi1_reg[UNITS1][UNITS1+UNITS2];
	#pragma HLS ARRAY_PARTITION variable=Wi1_reg dim=2 complete
	dtype bi1_reg[UNITS1];
	#pragma HLS ARRAY_PARTITION variable=bi1_reg complete
	dtype Wf1_reg[UNITS1][UNITS1+UNITS2];
	#pragma HLS ARRAY_PARTITION variable=Wf1_reg dim=2 complete
	dtype bf1_reg[UNITS1];
	#pragma HLS ARRAY_PARTITION variable=bf1_reg complete
	dtype Wc1_reg[UNITS1][UNITS1+UNITS2];
	#pragma HLS ARRAY_PARTITION variable=Wc1_reg dim=2 complete
	dtype bc1_reg[UNITS1];
	#pragma HLS ARRAY_PARTITION variable=bc1_reg complete
	dtype Wo1_reg[UNITS1][UNITS1+UNITS2];
	#pragma HLS ARRAY_PARTITION variable=Wo1_reg dim=2 complete
	dtype bo1_reg[UNITS1];
	#pragma HLS ARRAY_PARTITION variable=bo1_reg complete
	...
	LOOP_CELLS:for(int cl=0; cl<2; ++cl)
	{
		...
		for(int i=0, x=0, y=0; i< ((MAX_UNITS*MAX_MCOL*16)/512); i++, y++)
		{
			...
			for(int j=0;j<(512/32);j++)
			{
				#pragma HLS LOOP_TRIPCOUNT min = 16 max = 16
				#pragma HLS PIPELINE II = 1
				short int Wi_tmp, Wf_tmp, Wc_tmp, Wo_tmp;
				for(int z=0;z<2;z++)
				{
					...
					Wi1_reg[x][y*(512/16)+j*2+z]=(float)Wi_tmp/FRAQ_CONV;
					Wf1_reg[x][y*(512/16)+j*2+z]=(float)Wf_tmp/FRAQ_CONV;
					Wc1_reg[x][y*(512/16)+j*2+z]=(float)Wc_tmp/FRAQ_CONV;
					Wo1_reg[x][y*(512/16)+j*2+z]=(float)Wo_tmp/FRAQ_CONV;
				}
			}
		}
		for(int i = 0, x = 0 ; i< ((MAX_UNITS*16)/512); i++, x++)
		{
			...
			for(int j=0;j<(512/32);j++)
			{
				...
				for(int z=0;z<2;z++)
				{
					...
					bi1_reg[x*(512/16)+j*2+z]=(float)bi_tmp/FRAQ_CONV;
					bf1_reg[x*(512/16)+j*2+z]=(float)bf_tmp/FRAQ_CONV;
					bc1_reg[x*(512/16)+j*2+z]=(float)bc_tmp/FRAQ_CONV;
					bo1_reg[x*(512/16)+j*2+z]=(float)bo_tmp/FRAQ_CONV;
				}
			}
		}
		int pos=0;
		LOOP_TIMESTEPS:for(int k=0; k<actv_timesteps; ++k)
		{
			...
			input_gate(Wi1_reg, bi1_reg, x_h, i_t2);
			forget_gate(Wf1_reg, bf1_reg, x_h, f_t2);
			newcell_gate(Wc1_reg, bc1_reg, x_h, c_t2);
			output_gate(Wo1_reg, bo1_reg, x_h, o_t2);
